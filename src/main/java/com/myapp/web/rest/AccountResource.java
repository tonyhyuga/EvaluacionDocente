...
import com.myapp.service.PlanDeEstudiosService;
...
public class AccountResource {
    ...
    @Inject
    private PlanDeEstudiosService planService;
    ...
    public ResponseEntity<UserDTO> getAccount(HttpSession session) {
    	//System.out.println("Mi tiempo"+ session.getCreationTime());
    	Usuario usuario=userService.getUserWithAuthorities();
    	ResponseEntity<UserDTO> response = Optional.ofNullable(usuario)
         .map(user -> new ResponseEntity<>(new UserDTO(user), HttpStatus.OK))
         .orElse(new ResponseEntity<>(HttpStatus.INTERNAL_SERVER_ERROR));
    	if(response.getStatusCode().equals(HttpStatus.OK)){
    		if(usuario instanceof UsuarioEmpleado){
    			
    			session.setAttribute("Empleado", ((UsuarioEmpleado)usuario).getEmpleado());
    		}
    		if(usuario instanceof UsuarioAlumno){
    			AlumnoUADYMatriculado alumno=((UsuarioAlumno)usuario).getAlumnoUADYMatriculado();
    			PlanDeEstudios plan=planService.getPlanInscrito(alumno.getId());
    			//System.out.println(plan.getNombre());
    			session.setAttribute("Alumno", alumno);
    			session.setAttribute("PlanDeEstudios", plan);
    		}
    	}
    	else{
